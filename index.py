...
def cetakT():
    ...
    pdf.cell(200, 10, txt="Data Transaksi", ln=True, align='C')
    pdf.ln(10)

    pdf.set_font("DejaVu", "",size=8)
    pdf.set_fill_color(200, 220, 0)
    pdf.cell(20, 10, txt="Id Transaksi:",border=1, ln=0,fill=True)
    pdf.cell(20, 10, txt="Id Pasien:",border=1, ln=0,fill=True)
    pdf.cell(30, 10, txt="Nama Pasien:",border=1, ln=0,fill=True)
    pdf.cell(30, 10, txt="Kelas:",border=1, ln=0,fill=True)
    pdf.cell(30, 10, txt="Total Biaya:",border=1, ln=0,fill=True)
    pdf.cell(30, 10, txt="Status Pembayaran:",border=1, ln=0,fill=True)
    pdf.cell(30, 10, txt="Tanggal Transaksi:",border=1, ln=1,fill=True)


    pdf.set_fill_color(200, 220, 255)
    for d in data:
        pdf.cell(20, 10, txt=d['id_transaksi'], border=1)
        pdf.cell(20, 10, txt=d['id_pasien_rio'], border=1)
        pdf.cell(30, 10, txt=d['nama_rio'], border=1)
        pdf.cell(30, 10, txt=d['kelas_rio'], border=1, ln=0)
        pdf.cell(30, 10, txt=str(d['total_biaya_rio']), border=1, ln=0)
        pdf.cell(30, 10, txt=d['status_pembayaran_rio'], border=1, ln=0)
        tgl_str = d['tgl_transaksi_rio'].strftime('%Y-%m-%d') if hasattr(d['tgl_transaksi_rio'], 'strftime') else str(d['tgl_transaksi_rio'])
        pdf.cell(30, 10, txt=tgl_str, border=1, ln=1)
